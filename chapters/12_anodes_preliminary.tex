...
These results motivate the extension of dynamic impedance spectroscopy to battery formats that are closer to practical devices, while retaining the capability to discriminate the behavior of single electrodes under operating conditions.
...
The electrostatic distortion introduced by extended reference electrodes in compact pouch-cell geometries is schematically illustrated in Figure \ref{fig:micro reference positioning}
...
In lithium systems, micro-reference electrodes are achieved by in situ alloying of gold (in the form of wire) and lithium, whereas in sodium systems few attempts were made because its incapacity to alloy with gold. 
Sodium alloys instead with Tin but in a more complex multiphase chemistry.

In this expolarory work I could not replicate the alloying procedure found in the literature so I decided to procede with metal electrodeposition on copper wires for both Lithium and Sodium. 
...
An example of a fabricated micro-reference electrode integrated into a three-electrode pouch cell is shown in Figure \ref{fig: micro reference pouch}
...
The practical implementation of the micro-reference electrode in graphite/Li pouch cells is shown in Figure \ref{fig:graphite_pouch} and SiCN(O)/Na cells in Figure \ref{fig:SiCNO_pouch}.  
...
\begin{figure}[h]
    ...
    \caption{Post-mortem evidence of sodium deposition on SiCN(O).
...
\begin{figure}[h]
    ...
    Dynamic impedance during initial sodiation/desodiation, showing SEI formation behaviour comparable to the first cell.}
