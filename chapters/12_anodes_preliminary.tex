...
Dynamic impedance measurements have demonstrated a strong correlation with state of charge and state of health in commercial lithium-ion batteries, as established in Chapter~\ref{chap:coins}.  
In Chapters~\ref{chap:LTP} and \ref{chap:capacitor}, three-electrode flooded cells were used to isolate the impedance of individual electrodes and to relate the time-varying spectra to physically meaningful parameters.  
These results motivate the extension of dynamic electrochemical impedance spectroscopy (DEIS) to battery formats that are closer to practical devices, while retaining the capability to discriminate the behavior of single electrodes under operating conditions.

The next logical step was therefore to implement DEIS in pouch-cell--type three-electrode configurations.  
Pouch cells offer a high degree of geometric flexibility, larger electrode areas, and multilayer architectures, making them a standard platform for laboratory-scale battery development.  
Access to individual electrode processes during cycling would significantly support the design and testing of advanced materials, enabling identification of the electrode limiting performance and assisting the interpretation of degradation phenomena.

This chapter focuses on negative electrodes, because these are commonly associated with the onset of critical degradation mechanisms.  
...
DEIS is particularly suited for detecting such changes because the spectrum is acquired continuously during a galvanostatic or potentiostatic drift, allowing the observation of transient processes that conventional stationary EIS cannot capture.

To resolve the potential of a single electrode in a compact pouch-cell geometry, a miniaturized reference electrode is required.  
...
The overarching research questions guiding this work are:
\begin{enumerate}
    \item Can DEIS identify transient interfacial processes---such as SEI formation, staging phenomena, or metal plating---directly during electrode operation?
    \item Can characteristic impedance features be linked to specific mechanisms at negative electrodes for lithium- and sodium-based chemistries?
    \item What are the practical and methodological constraints associated with implementing three-electrode DEIS in pouch-cell systems?
\end{enumerate}
...
Its staging transitions, solid-electrolyte interphase (SEI) formation, and susceptibility to lithium plating under high-rate charging make it an ideal system for testing whether DEIS can resolve transient interfacial phenomena in situ.
...
These characteristics make SiCN(O) a promising negative electrode material for sodium-ion batteries, where hard carbon is currently the state of the art.

In contrast to graphite, SiCN(O) does not undergo intercalation into well-defined crystallographic galleries.  
Instead, several storage mechanisms have been proposed in the literature—including surface adsorption, pore filling, and reversible metal deposition within the nanoporosity.  
...
\subsubsection{Rationale for DEIS on SiCN(O)}

Two properties of SiCN(O) make it particularly interesting for dynamic impedance studies:
\begin{enumerate}
    \item its highly porous microstructure, which can host different Na storage modes that may evolve during a single half-cycle;
    \item experimental evidence from galvanostatic cycling (e.g.\ Ref.~[MdE]) suggesting that lowering the cutoff potential below~0~V vs.\ Na\textsuperscript{+}/Na triggers additional reversible capacity, possibly associated with sodium metal deposition within the pore network.
\end{enumerate}
...
Macroscopic reference electrodes, such as AgCl/Ag or metal strips, distort the potential distribution and introduce artefacts in the high- and mid-frequency regions, making them unsuitable for dynamic impedance measurements in multilayer or small-area cells.
...
\begin{figure}[h]
    \centering
    \includegraphics[width = 0.5\textwidth]{figures/anodes/reference electrode position.pdf}
    \caption{Cartoon depiting the distortion of the field lines at the border of a compact cell.}
    \label{fig:micro reference positioning}
\end{figure}

In lithium systems, microreference electrodes are achieved by in situ alloying of Gold (in the form of wire) and Lithium, whereas in sodium systems few attempts were made because its incapacity to alloy with gold. 
Soiudm alloys insted with Tin but in a more copmlex multiphase chemstry.
In this expolarory work I could not replicate the alloying procedure foud in the literature so I decided to procede with metal electrodeposition on copper wires for both Lithium and Sodium. 
The electrochemical stability of these microscopic electrodes is therefore strongly dependent on the quality of the insulation and the suppression of parasitic side reactions.  
Although the approach enabled DEIS measurements for extended periods, reproducibility remained limited because small imperfections in insulation, wire geometry, or deposition uniformity lead to drifts of the reference potential or intermittent artefacts.

\begin{figure}[h]
    \centering
    \includegraphics[width = \textwidth]{figures/anodes/micro reference electrode.pdf}
    \caption{Micro-reference electrode in pouch cell.}
...
In this chapter, only the essential aspects are reported, as the purpose of these measurements is not to optimise reference electrode design but to demonstrate that DEIS can be performed operando in three-electrode pouch cells.
% Explain why it is not so important to have a precise potential.

To verify the stability of the reference electrode during the measurement, I recorded the OCV betnween the reference electrode and the metallic counter electrode of the cells.
The voltage difference was always of 1-2 mV, conferming the stability for the experimetn time.
...
The synthesis route of SiCN(O), along with bulk characterization data and electrode formulation, is described in detail in MdE~\textit{On the reversible sodium plating} and is not repeated here.
...
The design follows the considerations developed in Chapter~\ref{chap:capacitor} and in the literature, which show that positioning the reference at the mid-plane minimize artifacts in the impedance measurements.  
...
The graphite-based three-electrode pouch cells are shown in Fig.~\ref{fig:graphite_pouch}, and the corresponding SiCN(O) cells in Fig.~\ref{fig:SiCNO_pouch}.  
In both cases the current-collector tabs were put in contact to aluminium or nickel leads without welding or other adesives, and the stack was enclosed in a laminated pouch and sealed under reduced pressure. 
...
\begin{figure}[H]
    ...
    \caption{Placement of the microscopic reference electrode (left) and completed three-electrode pouch cell (right) for graphite/Li measurements.}
...
\begin{figure}[H]
    ...
    \caption{Placement of the microscopic reference electrode (left) and completed three-electrode pouch cell (right) for SiCN(O)/Na measurements.}
    \label{fig:SiCNO_pouch}
\end{figure}

\section{DEIS measurement setup}

The dynamic impedance measurements were performed using the automated DEIS platform described in the experimental section.  
...
The online high-frequency impedance computation was not yet implemented at this stage, so the frequency band of the excitation was limited to 5 decades.  
...
\subsection{Applied drift and DEIS parameters for graphite}
...
\subsection{Applied drift and DEIS parameters for SiCN(O)}
SiCN(O) electrodes were tested under galvanostatic cycling between 0\,V and 2.5\,V vs.\ Na/Na$^+$, using a C/10 current defined relative to the theoretical capacity reported in MdE~\textit{On the reversible sodium plating}.
Being Silicon Carbonitrate an amorfous material, it is not possible to calculate the gravimetric maximum charge from its crycatl structure as for typical inorganic materials.
...
\subsection{Applied drift and DEIS parameters for metallic sodium}
...
For all measurements, the time-varying impedance was computed entirely off-line using the Dynamic Multi-Frequency Analysis (DMFA) method.  
A symmetric Fermi-Dirac window with bandwidth between 0.01\,Hz and 0.02\,Hz (selected according to the lowest excitation frequency) and order $n=8$ was employed.  
Linear baselines were removed from both current and voltage signals prior to filtering to eliminate slow drift and DC offsets.  
The resulting time resolution of the impedance is the inverse of the bandwidth: 100 seconds or 50 seconds.

\section{Results and Discussion}

\subsection{Graphite}

\subsubsection{General behaviour and formation}
Prior to the DEIS measurements, the graphite half-cells underwent a standard formation protocol followed by several galvanostatic cycles at 1C to ensure a stable SEI and reproducible electrochemical behaviour. The DEIS results reported here correspond to galvanostatic cycling at different C-rates (C/5, 1C, and 2C) within the voltage window of 0--0.8\,V vs.\ Li/Li$^+$. The theoretical capacity of the electrodes (11\,mAh) was used to set the applied current.

\subsubsection{Dynamic impedance at C/5}
...
\begin{figure}[H]
    ...
    \caption{Full C/5 galvanostatic cycle with DEIS. First spectrum and 10\,mHz point removed.}
...
This behaviour is consistent for both lithiation and delithiation and parallels the trends previously reported for the LTP system in Chapter~\ref{chap:LTP}, suggesting that changes in intercalation-induced porosity, tortuosity, or solid-phase diffusion influence the instantaneous impedance.
...
\begin{figure}[H]
    ...
    \caption{Localised transient reduction of impedance during C/5 lithiation.}
    \label{fig:graphite_feature_zoom}
\end{figure}
At this point the voltage simultaneously increases, implying a reduction in overpotential.
Although several interpretations are possible, one physically consistent hypothesis is the onset of a local, spatially heterogeneous overpotential regime, where some regions of the electrode may momentarily reach potentials favourable to lithium metal nucleation. 
In such conditions, the local charge-transfer resistance can decrease sharply, leading to an apparent impedance drop.
Importantly, the behaviour is only observed under cathodic current, reinforcing the interpretation that it is not purely geometrical or diffusive in origin.
Nevertheless, the signal-to-noise ratio worsens when the impedance becomes small relative to the perturbation amplitude, and this limits the precision of the estimation in this portion of the cycle.
Therefore, while suggestive, these observations must be interpreted cautiously and cannot be directly considered proof of plating.

\subsubsection{Effect of current rate}
At higher rates (1C and 2C), the behaviour differs markedly. As shown in Figure~\ref{fig:graphite_1C}, neither the staging-related changes in the low-frequency region nor the transient impedance reduction are observed. At 1C, the system does not reach the third intercalation plateau due to substantial overpotential, and only approximately 70\,\% of the theoretical capacity is accessed. 
...
\begin{figure}[H]
    \includegraphics[width = 0.5\textwidth]{figures/anodes/graphite_reduction_2C.png}
    \includegraphics[width = 0.5\textwidth]{figures/anodes/graphite_oxidation_2C.png}
    \caption{Dynamic impedance during 2C galvanostatic lithiation/delithiation.}
    \label{fig:graphite_2C}
\end{figure}

Figure~\ref{fig:graphite impedance comparison} compares the impedance at 50\,\% SOC for several C-rates. 
The asymmetry between lithiation and delithiation, previously documented for LTP, is also visible here. 
Additionally, the strong increase in high-frequency resistance and in the charge-transfer arc between 1C and 2C suggests that ???. 
...
The source of low precision are the poorer signal to noise ratio and strong intermodulation produced by non-linearity for mass trasnport controlled operational regime.
...
\begin{figure}[H]
    ...
    \caption{Comparison of instantaneous impedance at 50\,\% SOC for different C-rates.}
...
The SiCN(O) electrodes were tested from pristine state, so the first cycle includes the SEI formation, as shown in Figure~\ref{fig:SiCN cell 36 first cycle}. 
...
In the subsequent oxidation step, the impedance stabilises, and in the following cycle (Figure~\ref{fig:SiCN cell 36 second cycle}) the electrode behaviour is distinct from the formation impedance.  
...
\begin{figure}[H]
    ...
    \caption{First cycle of SiCN(O) at nominal C/10. SEI formation region marked by high impedance.}
...
\begin{figure}[H]
    ...
    \caption{Transient impedance reduction in SiCN(O) during the second cycle.}
    \label{fig:SiCN_feature_zoom}
\end{figure}
Because SiCN(O) is believed to support reversible sodium metal deposition within microporosity (as previously proposed in the literature), one hypothesis is that metallic sodium nucleates within microcavities once a critical local overpotential is reached. Such a transition could transiently reduce the charge-transfer resistance. Unlike in graphite, no simultaneous increase of the voltage is seen, which implies that the mechanism may be influenced by the specific surface chemistry of SiCN(O).
...
\begin{figure}[H]
    ...
    \caption{Post-mortem SiCN(O) electrode showing metallic sodium deposits.}
    \label{fig:SiCN post mortem}
\end{figure}

A repetition of the experiment (Figures~\ref{fig:SiCN cell 39 first cycle}--\ref{fig:SiCN cell 39 third cycle}) revealed similar SEI formation behaviour but did not show the transient impedance drop. This difference is plausibly attributable to variations in electrode porosity or local microstructure for the specific electrode.
Indeed Dr. Melzi D'Eril observed lack of reproducibility of the \emph{activation peak} for reversible electrodeposion (see the paper).
...
\subsubsection{Plating behaviour}
...
\begin{figure}[H]
    ...
    \caption{Dynamic impedance during sodium plating.}
...
Such positive imaginary features have been reported in systems where a slow intermediate step precedes nucleation or where SEI-mediated reconstruction controls early growth kinetics. 
The specific mechanism in sodium systems remains poorly documented, so the interpretation must remain tentative.

\subsubsection{Stripping behaviour}
...
\begin{figure}[H]
    ...
    \caption{Second sodium plating after prior stripping.}
...
\subsection{Mechanical intrusion and geometric artefacts of the reference electrode}

The micro-reference electrodes employed here had diameters comparable to those of the working electrodes (approximately $\sim50\,\mu$m). Even when used together with compliant glass-fibre separators, their thickness likely induced local deformation, disturbed the planarity of the electrode stack, and altered the current distribution—effects that are particularly severe in small-area pouch cells.

These geometric distortions may have influenced both the steady-state overpotential and the dynamic impedance response. Future implementations should consider:
...
\subsection{Pouch-cell sealing and environmental sensitivity}

Certain pouch cells exhibited incomplete sealing, which was only discovered a posteriori.  
In at least one case, an open seam in the pouch was identified, making air ingress unavoidable. Oxygen and moisture contamination are well known to trigger parasitic reactions with alkali metals and to alter SEI composition, thereby affecting both voltage profiles and impedance.
...
The SiCN(O) electrodes used in this study were characterised by meaningful microstructural variations related to their manufacturing process (porosity, binder distribution, carbon domain connectivity). Since impedance is intrinsically sensitive to surface condition and morphology, these variations produced:
...
\begin{itemize}
    ...
    \item uncertainty in attributing observed behaviour to fundamental electrochemistry rather than electrode heterogeneity.
\end{itemize}
...
This chapter demonstrates that dynamic electrochemical impedance spectroscopy can be successfully implemented in probing electrochemical processes \emph{in situ} under operating condition for half cells constituted of negative electrode material for Lihitum and Sodium ion storage.

DEIS measurements on graphite half-cells capturing staging transitions analogous to those observed for LTP in Chapter~\ref{chap:LTP} and at C/5 a reproducible transient impedance reduction appeared near the end of lithiation, speculated as lithium plating in region of the electrode of lower potential than the avarege.
Critically, this signature appeared only during cathodic galvanostatic techniques and disappeared at higher rates (1C, 2C), where concentration polarization dominates.
...
Post-mortem analysis confirmed metallic sodium deposits, supporting the hypothesis that sodium plating occurs under these conditions. 
The lack of reproducibility indicates sensitivity to electrode microstructure—particularly porosity and carbon-domain connectivity—and highlights the need for improved fabrication protocols.
On the positive note, an electroplated micro-reference electrode of metallic Sodium was reproduced multiple time and showed potential stability for dozen of hours, enough to complete multiple cycles of galvanostatic techniques even at low c-rates.
...
This chapter bridges the gap between the electrochemical characterization of single electrode (Chapters~\ref{chap:LTP} and \ref{chap:capacitor}) and the monitoring of aging of commercial full cells (Chapter~\ref{chap:coins}). 
...
Despite significant methodological challenges—particularly micro-reference electrode fabrication for lithium and limited reproducibility—the measurements reveal qualitatively consistent features that provide valuable mechanistic insights and motivate targeted future investigations.
In fact, not only this work demonstrate the capability of DEIS to resolve electrode-specific processes under different operating condition in three-electrode with newly designed micro-reference, but also showed that a transient impedance reduction apperas in both graphite and SiCN(O) appeared under comparable electrochemical conditions (low potentials, cathodic current) with similar spectral characteristics, suggesting an underlying physical origin worthy of systematic investigation.