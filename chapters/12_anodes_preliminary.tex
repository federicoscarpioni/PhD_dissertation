...
In this exploratory work I could not replicate the alloying procedure found in the literature so I decided to proceed with metal electrodeposition on copper wires for both Lithium and Sodium. 
...
The voltage difference was always of 1-2 mV, confirming the stability for the experiment time.
...
Although several interpretations are possible, one physically consistent hypothesis is the onset of a local, spatially heterogeneous overpotential regime, where some regions of the electrode may momentarily reach potentials favorable to lithium metal nucleation. 
...
This chapter demonstrates that dynamic electrochemical impedance spectroscopy can be successfully implemented to probe electrochemical processes \emph{in situ} under operating conditions in half-cells constituted of negative electrode materials for Lithium and Sodium ion storage.

DEIS measurements on graphite half-cells capturing staging transitions analogous to those observed for LTP in Chapter~\ref{chap:LTP} revealed that at C/5 a reproducible transient impedance reduction appeared near the end of lithiation, speculated as lithium plating in regions of the electrode where the potential is lower than the average.
...
On the positive note, an electroplated micro-reference electrode of metallic sodium was successfully reproduced multiple times and demonstrated potential stability for up to dozens of hours, enabling the completion of multiple cycles of galvanostatic techniques even at low C-rates.
...
This work demonstrates not only the capability of DEIS to resolve electrode-specific processes in three-electrode pouch cells under different operating conditions using newly designed micro-reference electrodes, but also reveals that a transient impedance reduction appears in both graphite and SiCN(O) under comparable electrochemical conditions (low potentials, cathodic current) with similar spectral characteristics, suggesting an underlying physical origin worthy of systematic investigation.