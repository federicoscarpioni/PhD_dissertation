...
When performing an experiment, an electronic instrumentation measures the quantity's value at a defined time interval using an electronic circuit called analog-to-digital converter (ADC).

In mathematical terms, the digital measurement system samples a continuous signal $x(t)$ at discrete times $t=nT_s$, producing the sampled discrete signal $x[n]$; where $T_s$ is the sampling time. 
