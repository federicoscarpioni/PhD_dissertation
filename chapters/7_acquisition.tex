...
This work gives a complete pipeline for the measurementes of Dynamic Ipedance Spectroscopy devided as:
\begin{itemize}
...
\end{itemize}
On a practical level I consider an experiment having three distincts but connected steps:
\begin{center}
    Acquisition $\rightarrow$ Estimation $\rightarrow$ Analysis,
\end{center}
therfore I devided this part of the thesis in three chapters beginning from the first one discussing the preparation and excecution of experiments with my contribution from the software side. 
Once multi-frequency voltage and current signlas of an electrochemical cells are acquired, the estimation can take place. This is the topic of next chapter (\ref{chap:estimation}).
...
% \begin{itemize}
%     \item \textbf{Section~\ref{sec:hardware}} describes the experimental setup made of indipendent devices;
...
The instrumental set-up used in our labs follows the same design introduced  in section \ref{sec:fft-eis} with a potentiostat interfaced with a dedicated waveform generator, for the excitation signal, and a dedicate acquisition device for high-frequency sampling and streaming to a computer. 
...
The extranal signal is applied before the \emph{memeasurement resistor}, so for current controlled experiments the current flowing though the circuit is calculated from the external applied voltage and the resitor with Ohm's law.
Not all commercial potentiostat have the capabilites to acsept external signals.
In this work I used some from the brand BioLogic which provides an (unsupported) library for controlling the instrument though open source programming languages.
This aspect was crucial for the automation of the Dynamic Impedance.

As arbitrary waveform generators we use dedicated dueal-channel benchtop devices from Keysight.
...
\end{itemize}
These device can be controlled with computer using the \emph{VISA} communication framework and \emph{SCIPI} langauge, both supported in Python with dedicated libraries.

The cell signals were sampled with USB acquisition devices from PicoTech that allow the capture of long continous signal via streaming capability at high samplig rates (up to 20MSa, though for electrochesmitr 1-3MSa is enough).
...
Figure \ref{fig:setup} shows a photography of one of the laboratory set-up used in this work (results of Chapter \ref{chap:coins}). 
...
Before I joined the ESECS group, the acquisition of multi-frequency cells signals was performed setting the three instruments manually and indipendelty, and streaming with a simple MATLAB script calling the PicoTech SDK.
...
Although it works for short measurment like the characterization of material provided in Chapters \ref{chap:LTP} and \ref{chap:capacitor}, my final goal was the monitoring of battery aging upon many cycles, therfore I needed to develop an alternative strategy: at the end of each half-cycle (or electrochemical technique) the signals from the RAM is stored to disk and the acquisition continue on the RAM.
This approach works until the signals of one half-cycle fit the memory, a constrain eliminated with the on-line computation that is the discussed in next Chapter.
To save the signals to disk at the right time, a computer program must follow the excecution of the technique on the potentiostat with a control loop, execute the saving to disk, and immediatly restart the data streaming without loosing samples.
This coordination coordination and the practical implementation is the subject of the rest of the chapter.

\subsection{Software solution}


A control loop must mointor the technique identifier and loop number returned by quering the potentiatiostat.
Using the BioLogic EC-Lab or EC-Lab Express doesn't allow to automate this process even using the output triggers.
BioLogic provides a simple Software Development Kit to communicate with the potentiostats that I expanded into a more sofisticated object-oriented Python library capable of communicating indipendently with each channel through a "Channel" object.
