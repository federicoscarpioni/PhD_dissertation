...
Once the impedance is measured, one can attempt to identify its dynamics by fitting a physical or non-physical mathematical model (parametric). The work presented later in this dissertation uses one type or the other. The physical models were all taken from the literature. We used a model for ion adsorption on nanoparticles to analyze the measurement of electrical double-layer capacitors and we adapted a model for intercalation with porous electrode theory, both developed inside the group in the previous years. The core measurement of this work was made on lithium-ion cells, which are not the classic analytical cells with reference electrodes commonly employed by electrochemists. The absence of the reference electrode makes it cumbersome to develop a physically derived model, so we decided to take the approach of a black-box model. Compared to time-domain characterization, where many black-box models exist for any type of system (ARMA, ARIMA, NARMAX, etc.), frequency-domain characterization lacks such variety and standardization, especially for time-varying systems.
Our group's approach is to estimate the instantaneous impedance at quasi-stationary points using dynamic multi-frequency analysis, thus treating the system as linear and time-invariant.
Therefore, the strategy is to fit a generic rational function of order $n$, inferring the correct structure of the transfer function of the system through statistical methods. 

\section{Physical model for intercalation in porous electrodes}

The impedance of intercalation materials is modeled using porous electrode theory, originally developed by De Levie \cite{deleviePorousElectrodesElectrolyte1963} for porous metal electrodes and later adapted by Bisquert \cite{bisquertInfluenceBoundaries2000} and others \cite{lamantiaImpedancePorous2008} for insertion battery materials. The model accounts for both the ionic transport through the electrolyte-filled pores and the charge transfer/intercalation processes at the electrode-electrolyte interface.
...
where $R_s$ is the uncompensated solution resistance, $R_p$ combines pore-solution and solid-particle resistances, and $kl$ embeds the finite diffusion or charge-transfer response of the porous electrode. This approach has been discussed in \cite{scarpioniElectrochemicalImpedanceSpectroscopy2023a}.
Figure \ref{fig:LTP circuit} shows the full equivalent circuit corresponding to the porous electrode model adopted in this work.

\begin{figure}[h]
    \includegraphics[width=\textwidth]{figures/LTP/ltp_circuit_new.png}
    \caption{Equivalent-circuit representation of the porous electrode model used to fit dynamic impedance spectra.
    The circuit captures ionic transport through electrolyte-filled pores, interfacial charge-transfer kinetics, double-layer and chemical capacitances, and distributed transport effects along the pore depth.
    This formulation follows a transmission-line approach commonly used for intercalation electrodes. \cite{scarpioniElectrochemicalImpedanceSpectroscopy2023a}}
    \label{fig:LTP circuit}
\end{figure}

The interface processes are described by the following simplified unit (Figure \ref{fig:LTP circuit simplified}) in the transmission line, where the Warburg element representing the charge-transfer has been substituted with a capacitor. This simplification produces better fits with fewer parameters.

\begin{figure}[h]
    \includegraphics[width=0.8\textwidth]{figures/LTP/EC_collins_simplified.jpg}
    \caption{Simplified interfacial unit used in the porous electrode transmission-line model.
    The diffusion-related Warburg element is replaced by an effective capacitive representation, reducing model complexity and improving parameter identifiability in dynamic impedance fitting.
    Adapted from the intercalation model developed in \cite{erinmwingbovoDynamicImpedanceSpectroscopy2020}.}
...
When impedance is measured for complex systems like batteries, where the electrodes' contributions cannot be separated via a reference electrode, or there are multiple layers of electrodes per cell, or even multiple cells are arranged in a battery pack, it is challenging to obtain a physical model from first principles.
An alternative is to use a measurement model, a mathematical function generic enough to represent the transfer function.

When looking at the problem from this perspective, the system is considered a black-box, and we depart from the knowledge of electrochemistry to draw from the framework of system identification of linear and time-invariant (LTI) systems.
...
\end{equation}
As discussed in Section \ref{sec:laplace}, the solution is obtained by applying the Laplace transform, which yields
\begin{equation}
    \sum_{N} a_n s^n Y(s) = \sum_{M} b_m s^m U(s)
\end{equation}
and hence the transfer function is given by
\begin{equation}
    G(s) = \frac{Y(s)}{U(s)}=\frac{\sum_{N} a_n s^n}{\sum_{M} b_m s^m}.
\end{equation}
This equation for the transfer function is a different form, but the same concept, as Equation \ref{eq:transfer function dy sys} from Section \ref{sec:transfer function def diff equations}.
...
The choice of penalty factor is somewhat arbitrary, but I found that values as small as $1 \times 10^{-12}$ are effective, although I expected to be ineffective since comparable to the round-up error of single-precision calculations.
Removing the regularization terms makes the optimization procedure ineffective or causes it to fail.

General measurement functions do not inform about the physics of the system, but they are a valuable approach to following the evolution of the parameters $a_n$ and $b_n$ with time.
In principle, it is possible to connect the parameters $a_n$ and $b_n$ of a rational polynomial with appropriate order $N$ to a physical model, at least for simple systems such as those modeled with Randles circuits \cite{gheemTheoreticalApproachModelling2008a}.
One can even try to find the best order $N$ via a model selection criterion and estimate the number of parameters needed in the model, then derive a physical model with the same degrees of freedom.