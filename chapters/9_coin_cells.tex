...
\marginpar{
  \centering
  \includegraphics[width=0.4\marginparwidth]{figures/results_coin/panasonic_ML2020_photo.jpg}
  \captionof{figure}{Panasonic LM2020 coin cell format battery used to test the long therm cyclation with dynamic impedance spectroscopy.}
  \label{fig:coin cell}
}
...
It can be observed at least two consecutive semicircles and a diffusive tail. At lower states of charge, represented in red, a third semicircle appears, shifting the diffusion to lower frequencies.
...
The cell is originally already charged from teh manifacturer. During the first cycle of the measurement, the cell voltage quickly reaches the charging limit and continues to discharge as shown in Figure~\ref{fig:first_cycle_coin}. 
...
The reproducibility of the dynamic impedance measurements across nominally identical cells is assessed in Figure~\ref{fig:eis_dyanmic_variability}.
...
For reference, the variability of stationary impedance measurements obtained during interrupted cycling is shown in Figure \ref{fig:eis_staitc_variability}.
...
A direct comparison between static and dynamic impedance measurements is presented in Figure~\ref{fig:eis_staitc_vs_dyanmic}.
...
Comparing the classic and non-stationray measurement it is clear how more convenient is the multisien perturbation over the single sine, as reported in Figure \ref{fig:static vs multisine experiment}.
The latter takes around 27 hours to produce 10 spectra while the multisine based method outputs in 10 hours 270 spectra.
...
Parameters $a_1$ (orange line) and $a_3$ (red line) for jump between values because a new feature in the impedance becomes significant: a semicircle appears only at high state of charge and then smoothly disappears into another semicircle at high frequency.
...
As seen in Figure \ref{fig:deis_aging_charge} and \ref{fig:deis_aging_discharge} after cycle 5 the shape of the impedance tend to stabilizes, showing two consecutive semicircles and a diffusive feature.
This is visible in the resulting fitting parameters for cycle 5 \ref{fig:fitting_parameters_cycle5} and successive (11 in Figure \ref{fig:fitting_parameters_cycle11} and 20 in Figure \ref{fig:fitting_parameters_cycle20}).
The parameters shapes and magnitude is preserved across cycles and they are mostly consecutive through the techniques of the same cycle.
Therefore the parameters seems correlated with the state of charge and state of health of the battery.
The same behavior is consistent with the other experiment of the same type.
