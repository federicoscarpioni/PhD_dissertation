...
\chapter{Exploratory anode studies with DEIS}
...
\begin{enumerate}
    ...
    \item experimental evidence from galvanostatic cycling (e.g.\ Ref.~[MdE]) suggesting that lowering the cutoff potential below~0~V vs.\ Na/Na\textsuperscript{+} triggers additional reversible capacity, possibly associated with sodium metal deposition within the pore network.
\end{enumerate}
...
Being Silicon Carbonitrate an amorfoud material, it is not possible to calculate the gravimetric maximum charge from its crycatl structure as for typical inorganic materials.
